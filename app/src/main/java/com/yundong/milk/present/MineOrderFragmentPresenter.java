...
import com.yundong.milk.interaptor.impl.ConfirmReceiveGoodsImpl;
...
public class MineOrderFragmentPresenter {
    ...
    //确认收货
    private ConfirmReceiveGoodsImpl confirmReceiveGoods;
    private IConfirmReceiveGoodsView iConfirmReceiveGoodsView;

    private MineOrderFragmentPresenter() {
        orderList = new OrderListImpl();
        cancleOrder = new CancleOrderImpl();
        confirmReceiveGoods=new ConfirmReceiveGoodsImpl();
    }
    ...
    public MineOrderFragmentPresenter with(IConfirmReceiveGoodsView iConfirmReceiveGoodsView) {
        this.iConfirmReceiveGoodsView = iConfirmReceiveGoodsView;
        return maineOrderActivityPresenter;
    }
    ...
    //确认收货
    public void confirmReceiveGoods(String order_id) {
        confirmReceiveGoods.confirmReceiveGoods(order_id)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<BaseReceiveBean>() {
                    @Override
                    ...
                    @Override
                    public void onError(Throwable e) {
                        iConfirmReceiveGoodsView.confirmReceiveGoodsOnError(e.getMessage());
                    }

                    @Override
                    public void onNext(BaseReceiveBean baseReceiveBean) {
                        iConfirmReceiveGoodsView.confirmReceiveGoods(baseReceiveBean);
                    }
                });
    }
}
